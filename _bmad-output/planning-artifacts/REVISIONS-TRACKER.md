...
- [ ] **PX-022b-FUP-1 — Sally's visual design pass.** Replace the programmatic (Pillow+fabric) templates + thumbnails with hand-designed variants. Current set is functional, license-safe, and tag-rich but visually plain. Size: L (~1 day Sally + authoring rework). Priority: P2 (improves gallery quality-of-first-impression before MVP external review).
- [ ] **PX-022b-FUP-2 — First-class `Logo` platform (if product wants).** Decide: extend `PlatformLiteral` to include `'logo'` (touches `schemas/template.py`, `backend/app/core/platform_presets.py`, `src/app/core/constants/platform-presets.ts`, parity test) OR add `category: 'logo'` field on Template. Current MVP surfaces logos via `tags: ['Logo']` on ig-post — works but doesn't give a dedicated gallery tab. Size: S. Priority: P3 (product decision first).
