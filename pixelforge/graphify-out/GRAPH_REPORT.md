...
## Corpus Check
- 141 files · ~269,010 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1575 nodes · 3044 edges · 45 communities detected
- Extraction: 60% EXTRACTED · 40% INFERRED · 0% AMBIGUOUS · INFERRED: 1211 edges (avg confidence: 0.76)
...
5. `PropertyPanelComponent` - 47 edges
...
Cohesion: 0.03
Nodes (7): ClipboardService, ColorPalettePanelComponent, ContextMenuComponent, GradientPanelComponent, ImageFiltersPanelComponent, PropertyPanelComponent, TextToolbarComponent

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (8): AccessibilityService, AlignmentPanelComponent, CanvasService, makeService(), Canvas, LayerPanelComponent, ResizeDialog, StyleVariationsService
...
Cohesion: 0.08
Nodes (8): ExportDialog, ExportService, setup(), StaticCanvas, templateFactory(), normalizeHex(), remapObject(), TemplateThumbnailService
...
Cohesion: 0.08
Nodes (4): FontService, QualityScoreService, TemplateService, ThemeService
...
- **Thin community `Community 20`** (20 nodes): `.addReply()`, `CommentsOverlay`, `.cancelNewComment()`, `.closeMention()`, `.closeThread()`, `.deleteComment()`, `.formatMentions()`, `.knownUsers()`, `.ngOnDestroy()`, `.ngOnInit()`, `.onNewEnter()`, `.onReplyInput()`, `.onReplyKey()`, `.pickMention()`, `.screenX()`, `.screenY()`, `.submitNewComment()`, `.submitReply()`, `.toggleComment()`, `comments-overlay.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (17 nodes): `AnimationService`, `.animate()`, `.easeOutBounce()`, `.easeOutCubic()`, `.resetAll()`, `.setAnimation()`, `.snapshot()`, `AnimationTimeline`, `.attachWhenReady()`, `.getAnimIcon()`, `.getAnimLabel()`, `.msToPixels()`, `.ngOnDestroy()`, `.ngOnInit()`, `.quickAddAnim()`, `animation.service.ts`, `animation-timeline.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (15 nodes): `Canvas`, `.dispose()`, `.getObjects()`, `.getZoom()`, `.off()`, `.on()`, `.renderAll()`, `.requestRenderAll()`, `.setDimensions()`, `.setViewportTransform()`, `.setZoom()`, `constructor()`, `FabricObject`, `getObjects()`, `export.service.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (13 nodes): `AiBackgroundService`, `.buildSvg()`, `.detectType()`, `.generate()`, `.generateAndApply()`, `.matchPalette()`, `AiDesignService`, `.generate()`, `.generateHeadline()`, `.generateSubheadline()`, `.parsePrompt()`, `ai-background.service.ts`, `ai-design.service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (11 nodes): `MagicWriteService`, `.applySwaps()`, `.generateVariants()`, `.makeLonger()`, `.makeShorter()`, `.removeFillers()`, `.sentenceCase()`, `.titleCase()`, `.toHeadline()`, `.transform()`, `magic-write.service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 32`** (6 nodes): `btn()`, `heading()`, `mkAuthResponse()`, `pills()`, `setup()`, `auth.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (6 nodes): `PwaInstallPrompt`, `.dismiss()`, `.install()`, `.ngOnDestroy()`, `.ngOnInit()`, `pwa-install-prompt.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `seed_templates()` connect `Community 5` to `Community 9`, `Community 2`?**
