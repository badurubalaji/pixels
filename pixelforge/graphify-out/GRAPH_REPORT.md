...
## Corpus Check
- 1523 files · ~99,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1523 nodes · 2460 edges · 51 communities detected
- Extraction: 71% EXTRACTED · 29% INFERRED · 0% AMBIGUOUS · INFERRED: 704 edges (avg confidence: 0.76)
...
- [[_COMMUNITY_Auth Core (JWThash) + Profile Edit (PX-071)|Auth Core (JWT/hash) + Profile Edit (PX-071)]]
...
- [[_COMMUNITY_Auth Page Component (PX-063)|Auth Page Component (PX-063)]]
...
Cohesion: 0.03
Nodes (9): AnimationTimeline, BackgroundRemovalService, ColorPalettePanelComponent, FabricObject, GradientPanelComponent, ImageFiltersPanelComponent, PropertyPanelComponent, PwaInstallPrompt (+1 more)

### Community 1 - "Canvas Service Core"
Cohesion: 0.02
Nodes (92): ApiService, ConnectionManager, project_collab_socket(), Real-time collaboration via WebSocket.  Each project has a "room". Connected cli, Tracks active websocket connections per project room., ProjectCreate, ProjectDetailResponse, ProjectResponse (+84 more)
...
Cohesion: 0.03
Nodes (11): CollaborationService, DesignHelperService, Editor, setup(), StaticCanvas, templateFactory(), KeyboardService, normalizeHex() (+3 more)
...
Cohesion: 0.03
Nodes (8): AccessibilityService, AnimationService, ClipboardService, ContextMenuComponent, Canvas, QualityScoreService, QuickActionBar, StyleVariationsService
...
### Community 7 - "Auth Core (JWT/hash) + Profile Edit (PX-071)"
Cohesion: 0.03
...
Cohesion: 0.06
Nodes (59): AuthResponse, create_token(), decode_token(), get_current_user(), hash_password(), Partial-update payload for the authenticated user's own profile.      Fields def, Returns user dict if authenticated, None otherwise (optional auth)., Requires authenticated user or raises 401. (+51 more)
...
Cohesion: 0.1
Nodes (4): AiBackgroundService, AiDesignService, TemplateService, ThemeService
...
Cohesion: 0.12
Nodes (15): Happy-path coverage for auth_routes endpoints after the Depends refactor., POST /api/auth/login returns a token for a previously-created user., GET /api/auth/me echoes the token's user., PATCH /api/auth/me writes the new name and returns it (PX-071)., Whitespace-only or empty name clears the field (stored as None, PX-071)., Names over 60 characters are rejected with 400 (PX-071)., POST /api/auth/signup creates user and returns a token., PATCH /api/auth/me without a token returns 401 (PX-071). (+7 more)

### Community 22 - "Community 22"
Cohesion: 0.17
Nodes (1): CanvasRulersComponent
...
### Community 27 - "Auth Page Component (PX-063)"
Cohesion: 0.2
...
- **Thin community `Community 18`** (20 nodes): `.addReply()`, `CommentsOverlay`, `.cancelNewComment()`, `.closeMention()`, `.closeThread()`, `.deleteComment()`, `.formatMentions()`, `.knownUsers()`, `.ngOnDestroy()`, `.ngOnInit()`, `.onNewEnter()`, `.onReplyInput()`, `.onReplyKey()`, `.pickMention()`, `.screenX()`, `.screenY()`, `.submitNewComment()`, `.submitReply()`, `.toggleComment()`, `comments-overlay.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 19`** (19 nodes): `.updateProject()`, `BrandKitApplyService`, `._buildSubstitutionMap()`, `.clearMarker()`, `._clearMarkerServerSide()`, `._revertObjectColors()`, `.revertToTemplateDefaults()`, `.finishNameEdit()`, `.toJSON()`, `HistoryService`, `.clear()`, `.init()`, `.redo()`, `.restoreState()`, `.saveState()`, `.undo()`, `.updateCounts()`, `brand-kit-apply.service.ts`, `history.service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (16 nodes): `.restoreVersion()`, `CanvasRulersComponent`, `.drawHorizontalRuler()`, `.drawRulers()`, `.drawVerticalRuler()`, `.getGuideScreenPos()`, `.getStep()`, `.ngAfterViewInit()`, `.onMouseMove()`, `.onMouseUp()`, `.removeGuide()`, `.startDragGuide()`, `.startMoveGuide()`, `.toggleRulers()`, `canvas-rulers.ts`, `.restore()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 30`** (9 nodes): `HubComponent`, `.buildTiles()`, `.ngOnInit()`, `.onProjectActivate()`, `.onStartFromScratch()`, `.onTileActivate()`, `.trackProjectById()`, `.trackTileById()`, `hub.component.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 31`** (8 nodes): `AlignmentPanelComponent`, `.align()`, `.attachWhenReady()`, `.autoArrange()`, `.constructor()`, `.distribute()`, `.fixOverlaps()`, `alignment-panel.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (7 nodes): `authGuard()`, `FakeAuthService`, `.isAuthenticated()`, `.setAuthenticated()`, `runGuard()`, `auth.guard.spec.ts`, `auth.guard.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Hub Component (PX-064)`** (7 nodes): `template-thumbnail.service.spec.ts`, `StaticCanvas`, `.constructor()`, `.dispose()`, `.loadFromJSON()`, `.renderAll()`, `.toDataURL()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (6 nodes): `btn()`, `heading()`, `mkAuthResponse()`, `pills()`, `setup()`, `auth.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (6 nodes): `LayerPanelComponent`, `.onDrop()`, `.setLayerOpacity()`, `.toggleLock()`, `.toggleVisibility()`, `layer-panel.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (5 nodes): `CollabOverlay`, `.initial()`, `.screenX()`, `.screenY()`, `collab-overlay.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 43`** (4 nodes): `__init__.py`, `__init__.py`, `__init__.py`, `Pydantic v2 schemas for request/response DTOs and domain documents.  Sub-modules`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (4 nodes): `BackgroundPanelComponent`, `.onCustomColor()`, `.onModeChange()`, `background-panel.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (2 nodes): `run.py`, `Development server entry point.  Usage:     python run.py     # or     uvicorn a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (2 nodes): `test-setup.ts`, `TestRootModule`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (2 nodes): `types.d.ts`, `GIF`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (2 nodes): `App`, `app.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (2 nodes): `ShortcutsDialog`, `shortcuts-dialog.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 63`** (1 nodes): `Require a leading ``#`` followed by 3 or 6 hex digits.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 64`** (1 nodes): `Require ``thumbnail_data_url`` to start with ``data:image/``.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FabricObject` connect `Animation Service` to `AI Background Service`, `Community 5`, `Canvas Service Core`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `seed_templates()` connect `Brand Kit Service` to `Canvas Service Core`, `Community 11`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `author_all()` connect `Dashboard Component (PX-068/069)` to `Animation Service`, `Community 12`?**
