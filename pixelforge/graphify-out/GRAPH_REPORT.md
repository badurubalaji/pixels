...
## Corpus Check
- 1604 files · ~99,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1604 nodes · 2985 edges · 55 communities detected
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 1130 edges (avg confidence: 0.76)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- [[_COMMUNITY_Animation Service|Animation Service]]
- [[_COMMUNITY_Canvas Service Core (PX-090..101)|Canvas Service Core (PX-090..101)]]
...
- [[_COMMUNITY_Editor (PX-072..101)|Editor (PX-072..101)]]
...
### Community 1 - "Canvas Service Core (PX-090..101)"
Cohesion: 0.03
Nodes (10): AuthComponent, AuthService, ContextMenuComponent, Dashboard, ImageFiltersPanelComponent, ProfileComponent, PwaInstallPrompt, ResizeDialog (+2 more)

### Community 2 - "Project Models & Routes"
Cohesion: 0.04
Nodes (6): AlignmentPanelComponent, AnimationService, ClipboardService, ColorPalettePanelComponent, PropertyPanelComponent, QuickActionBar

### Community 3 - "Seed & Migrations"
Cohesion: 0.04
Nodes (5): CanvasService, Canvas, LayerPanelComponent, StyleVariationsService, TemplateService
...
- **Thin community `Community 25`** (15 nodes): `GradientPanelComponent`, `.addStop()`, `.applyGradient()`, `.applyPreset()`, `.attachWhenReady()`, `.getPresetGradientCSS()`, `.ngOnDestroy()`, `.ngOnInit()`, `.removeStop()`, `.setAngle()`, `.setFillType()`, `.startEyedropper()`, `.updateStopColor()`, `.updateStopOffset()`, `gradient-panel.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (12 nodes): `.setAnimation()`, `AnimationTimeline`, `.attachWhenReady()`, `.getAnimIcon()`, `.getAnimLabel()`, `.msToPixels()`, `.ngOnDestroy()`, `.ngOnInit()`, `.playAll()`, `.quickAddAnim()`, `.previewAnimation()`, `animation-timeline.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 32`** (10 nodes): `ColorPickerComponent`, `.commitHexInput()`, `.composeHex()`, `.emitChange()`, `.onAlphaChange()`, `.onHexChange()`, `.onHexInputChange()`, `.parseColor()`, `.value()`, `color-picker.component.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Hub`** (9 nodes): `BackgroundRemovalService`, `.dataURLToBlob()`, `.removeBackground()`, `.removeClientSide()`, `.removeFromDataURL()`, `.removeServerSide()`, `.reset()`, `.triggerBgRemoveUpload()`, `background-removal.service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 34`** (9 nodes): `.updateProject()`, `BrandKitApplyService`, `._buildSubstitutionMap()`, `.clearMarker()`, `._clearMarkerServerSide()`, `._revertObjectColors()`, `.revertToTemplateDefaults()`, `.finishNameEdit()`, `brand-kit-apply.service.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 35`** (9 nodes): `HubComponent`, `.buildTiles()`, `.ngOnInit()`, `.onProjectActivate()`, `.onStartFromScratch()`, `.onTileActivate()`, `.trackProjectById()`, `.trackTileById()`, `hub.component.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `author_all()` connect `Community 18` to `Canvas Service Core (PX-090..101)`, `Seed & Migrations`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `seed_templates()` connect `Community 5` to `Animation Service`, `Database + FastAPI Bootstrap`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `test_each_doc_meets_palette_and_tag_requirements()` connect `Community 5` to `Canvas Service Core (PX-090..101)`?**
...
- **Should `Canvas Service Core (PX-090..101)` be split into smaller, more focused modules?**
  _Cohesion score 0.03 - nodes in this community are weakly interconnected._
