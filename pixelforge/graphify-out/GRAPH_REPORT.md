...
## Corpus Check
- 1503 files · ~99,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1503 nodes · 2388 edges · 51 communities detected
- Extraction: 73% EXTRACTED · 27% INFERRED · 0% AMBIGUOUS · INFERRED: 654 edges (avg confidence: 0.77)
...
Cohesion: 0.03
Nodes (10): AnimationService, ClipboardService, ColorPalettePanelComponent, ContextMenuComponent, Canvas, setup(), StaticCanvas, templateFactory() (+2 more)

### Community 1 - "Canvas Service Core"
Cohesion: 0.03
Nodes (8): AuthComponent, AuthService, BackgroundRemovalService, GradientPanelComponent, HubComponent, ImageFiltersPanelComponent, PwaInstallPrompt, TextToolbarComponent

### Community 2 - "Project Models & Routes"
Cohesion: 0.02
Nodes (59): ApiService, BrandKitApplyService, ConnectionManager, project_collab_socket(), Real-time collaboration via WebSocket.  Each project has a "room". Connected cli, Tracks active websocket connections per project room., CommentsService, ShareDialog (+51 more)
...
Cohesion: 0.04
Nodes (6): Editor, KeyboardService, normalizeHex(), remapObject(), TemplateThumbnailService, ToolbarPanelComponent
...
Cohesion: 0.05
Nodes (47): AuthResponse, decode_token(), get_current_user(), Returns user dict if authenticated, None otherwise (optional auth)., Requires authenticated user or raises 401., require_user(), UserLogin, UserPublic (+39 more)
...
Cohesion: 0.06
Nodes (3): Dashboard, ProfileComponent, UserMenuComponent

### Community 12 - "Community 12"
Cohesion: 0.14
Nodes (35): ProjectCreate, ProjectDetailResponse, ProjectResponse, ProjectUpdate, Request / response Pydantic v2 models for the core project + asset APIs.  PX-060, Incoming payload for ``POST /api/projects``.      Attributes:         name: Disp, Partial update payload for ``PUT /api/projects/{id}``.      All fields are optio, List-view projection of a project document.      Excludes the heavyweight ``canv (+27 more)
...
Cohesion: 0.1
Nodes (4): AiBackgroundService, AiDesignService, DesignHelperService, MagicWriteService

### Community 15 - "Community 15"
Cohesion: 0.09
Nodes (4): AuditDialog, CommandPalette, NewProjectDialog, ResizeDialog

### Community 16 - "Dashboard Component"
Cohesion: 0.1
Nodes (3): AlignmentPanelComponent, AnimationTimeline, HistoryService
...
- **Thin community `Community 18`** (25 nodes): `.restoreVersion()`, `CanvasRulersComponent`, `.drawHorizontalRuler()`, `.drawRulers()`, `.drawVerticalRuler()`, `.getGuideScreenPos()`, `.getStep()`, `.ngAfterViewInit()`, `.ngOnDestroy()`, `.onMouseMove()`, `.onMouseUp()`, `.removeGuide()`, `.startDragGuide()`, `.startMoveGuide()`, `.toggleRulers()`, `CollaborationService`, `.connect()`, `.disconnect()`, `.handleMessage()`, `.send()`, `.sendCanvasUpdate()`, `.sendCursor()`, `collaboration.service.ts`, `canvas-rulers.ts`, `.restore()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `seed_templates()` connect `Brand Kit Service` to `Database + FastAPI Bootstrap`, `Community 12`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `test_each_doc_meets_palette_and_tag_requirements()` connect `Brand Kit Service` to `Canvas Service Core`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `FabricObject` connect `Community 13` to `Animation Service`, `Canvas Service Core`, `Project Models & Routes`, `Editor Component`?**
